...
- **Une CI externe est invisible via l'API.** Xcode Cloud, Codemagic et consorts ne laissent aucune trace exploitable : `DuoScribo`, `FeuilleBlanche` et `nannyplus` seraient marqués `KO` à tort sur CI-01/CI-05. La présence de `ci_scripts/` prouve Xcode Cloud, mais son absence ne prouve rien — signal inutilisable seul. C'est ce que `.repocheckignore` doit résoudre.
